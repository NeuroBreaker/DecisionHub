...
async def Member_registration_service(
    fio: str, group_name: str, password: str, email: str, role, link="No info"
):
    try:
        ...
        if role.upper() == "PARTICIPANT":
            logger.info("Table CRUD attempt")
            try:
...
async def Doc_analytic(file, words_list, min_char=100):
    ...
    try:
        ...
        logger.error(
            f"Ошибка во время анализа документа {file.filename if file else 'unknown'}: {e}"
        )
        return {
...
async def doc_git_analytic(url: str, words_list: list) -> dict:
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(zip_data)) as z:
            ...
            has_readme = any(
                "readme.md" in f.lower() or "readme.txt" in f.lower() for f in namelist
            )
            has_license = any("license" in f.lower() for f in namelist)
            has_deps = any(
                "requirements.txt" in f
                or "package.json" in f
                or "pyproject.toml" in f
                or "go.mod" in f
                or "pom.xml" in f
                for f in namelist
            )
            has_startup_instruction = any(
                ...
                or "dockerfile" in f.lower()
                or "makefile" in f.lower()
                or "setup.sh" in f.lower()
                for f in namelist
            )
            ...
            code_files = [
                f
                for f in namelist
                if f.endswith((".py", ".js", ".ts", ".go", ".java", ".cpp"))
            ]
            for file_path in code_files[:25]:
                ...
                file_complexity = 1
                file_complexity += len(
                    re.findall(r"\b(if|for|while|except|catch|elif)\b", content)
                )
                file_complexity += (
                    content.count("&&")
                    + content.count("||")
                ...
                for pattern in secret_patterns:
                    ...
                    for match in matches:
                        ...
                        detected_secrets.append(
                            f"{file_path.split('/')[-1]}: найден секрет ({clean_match})"
                        )

            avg_complexity = (
                round(total_complexity / analyzed_files_count, 1)
            ...
            if has_deps:
                structure_score += 2.5
            if has_startup_instruction:
                structure_score += 2.5

            secret_penalty = min(len(detected_secrets) * 2, 4)
            ...
            structure_status = {
                ...
                "dependency_file_exists": "✅ Найдено"
                if has_deps
                else "❌ Отсутствует",
                "startup_instructions_exists": "✅ Найдено"
                if has_startup_instruction
                else "❌ Отсутствует",
            }
